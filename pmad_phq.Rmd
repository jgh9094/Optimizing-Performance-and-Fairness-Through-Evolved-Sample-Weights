# PMAD PHQ

Here we report the **hypervolume** achived by evaluating the performance of each solution wihtin the Pareto front on the test set of the `pmad_phq` dataset.

```{r pp-hv-data, message=FALSE}
# heart-disease data
data <- filter(testing, dataset == "pmad_phq")
```

## Hypervolume

```{r pp-hv-plt, echo=TRUE, message=FALSE, out.width="100%"}
volume_plotter(data,13)
```

### Summary stats

```{r pp-hv-stats, echo=TRUE, message=FALSE, out.width="100%"}
volume_summarize(data)
```

### Kruskal-Wallis test

Detected differences between weight strategies.

```{r pp-hv-kru, echo=TRUE, message=FALSE, out.width="100%"}
kruskal.test(hv ~ exp, data = data)
```

### Pairwise wlcoxon test

```{r pp-hv-wil, echo=TRUE, warning=FALSE}
pairwise.wilcox.test(x = data$hv, g = data$exp, p.adjust.method = "bonferroni",
                    paired = FALSE, conf.int = FALSE, alternative = 'l')
```